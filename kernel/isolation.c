 ...
 *
 * We need to run in the return to user path so we are late enough to
 * guarantee no further kernel things will happen to perturb our
 * carefully-arranged isolation state that includes dyntick off,
 * per-cpu lru cache empty, and the like.
 *
 * Note that we don't explicitly try to quiesce the dyntick, since
 * there's very little we can usefully do.  If we're just waiting for
 * the dyntick, we'll keep looping around in the code that calls
 * task_isolation_ready() and task_isolation_prepare() until the
 * last dyntick fires.
 */
void task_isolation_prepare(void)
{
	WARN_ON_ONCE(irqs_disabled());
			       ...
			       const char *fmt, va_list args)
{
	...
	if (sig != 0) {
		if (in_nmi())
			...
			task_isolation_deliver_signal(p, buf, sig);
	}
