				     ...
				     struct pt_regs *regs)
{
	...
	if (i == ARC_MAX_HBP_SLOTS) {
		pr_err("BP do not match any HW breakpoint\n");
		return;
	}

	num = i + 1;
	/* mark as breakpoint if target instruction address */
	if (bkpt->ctrl.target == ARC_BREAKPOINT_TARGET_INST_ADDR)
		num = 0 - num;
	...
	case PTRACE_POKEUSR:
		...
		if (addr >= offsetof(struct user, u_dr_value[0]) &&
			addr <= offsetof(struct user,
					 u_dr_value[ARC_MAX_HBP_SLOTS-1])) {
			...
			addr <= offsetof(struct user,
					 u_dr_control[ARC_MAX_HBP_SLOTS-1])) {
