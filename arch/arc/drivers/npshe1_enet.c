...
static const struct net_device_ops nps_he1_netdev_ops = {
...
static int __devinit ez_net_probe( struct platform_device *pdev )
{
	...
	netdev->netdev_ops     = &nps_he1_netdev_ops;
...
static int __devexit ez_net_remove( struct platform_device *pdev )
{
	...
	free_irq( IRQ_LAN_RX_LINE, npshe1dev );
	free_irq( IRQ_LAN_TX_LINE, npshe1dev );
...
static struct platform_driver npshe1_lan_driver = {
   .probe      = ez_net_probe,
...
static int __init npshe1_enet_init_module(void)
{
	int rc;
...
static void __exit npshe1_enet_cleanup_module(void)
{
   platform_driver_unregister(&npshe1_lan_driver);
}


module_exit(npshe1_enet_cleanup_module);
module_init(npshe1_enet_init_module);
