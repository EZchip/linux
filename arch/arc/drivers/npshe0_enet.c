...
struct net_device *npshe0dev;
extern struct sockaddr mac_addr;	/* Intialised while in setup.c */
...
{
	int res;
	EZT_NET_PRIV *netPriv= netdev_priv(netdev);
	struct sockaddr *x = (struct sockaddr *)p;
	char *uc_addr = (char *)x->sa_data;

	netif_info(netPriv, hw, netdev,"new MAC addr %02x:%02x:%02x:%02x:%02x:%02x\n",
...
{
	REG_GEMAC_TX_CTL &= ~(TX_CTL_RESET);
	REG_GEMAC_RX_CTL &= ~(RX_CTL_RESET);
}
...
static void ez_net_send_buf(struct net_device *netdev, struct sk_buff *skb,short length)
{
	...
	if (((int)srcData & 0x3) == 0) {
		for(i = 0; i < k; i++)
			REG_GEMAC_TXBUF_DATA = srcData[i];
	} else { /* Not aligned */
		for(i = 0; i < k; i++){
			/*
			 * Tx buffer is a FIFO
			 ...
			 */
			memcpy(&dummy_buf,srcData+i,EZ_NET_REG_SIZE);
			REG_GEMAC_TXBUF_DATA = dummy_buf;
		}
	}
	/* Write the length of the Frame */
	REG_GEMAC_TX_CTL = (REG_GEMAC_TX_CTL & ~(TX_CTL_LENGTH_MASK)) | length;
	/* Send Frame */
	REG_GEMAC_TX_CTL |= TX_CTL_BUSY;
...
static const struct net_device_ops nps_he0_netdev_ops = {
...
static void clean_rx_fifo(void)
{
	int i;
	int length;
	int k;

	length = REG_GEMAC_RX_CTL & RX_CTL_LENGTH_MASK;
	k = array_int_len(length);
	for(i = 0; i < k; i++)
		length = REG_GEMAC_RXBUF_DATA;

}

static void read_rx_fifo(EZT_NET_PRIV *netPriv, unsigned int *dest)
{
	int i;
	int length;
	...
	if (((int)dest & 0x3) == 0) {
		for(i = 0; i < k; i++)
			dest[i] = REG_GEMAC_RXBUF_DATA;
	} else {
		for(i = 0; i < k; i++) {
			dummy_buf = REG_GEMAC_RXBUF_DATA;
			memcpy(dest+i,&dummy_buf,EZ_NET_REG_SIZE);
		}
	}
}
...
static irqreturn_t ez_net_rx_irq(int irq, void *dev_id)
{
	...
	if (unlikely(frame_len > EZ_NET_LAN_BUFFER_SIZE || frame_len < ETH_ZLEN)){
		netdev->stats.rx_dropped++;
    	goto rx_irq_error;
	}
    ...
    read_rx_fifo(netPriv, (unsigned int*)skb->data);
...
static irqreturn_t ez_net_tx_irq(int irq, void *dev_id)
{
	...
	/* Check Tx error */
    if (unlikely(REG_GEMAC_TX_CTL & TX_CTL_ERROR) != 0 ) {
    	netdev->stats.tx_errors++;
    	/* clean TX CTL error */
    	REG_GEMAC_TX_CTL &= ~(TX_CTL_ERROR);
    } else {
...
static int __devinit ez_net_probe( struct platform_device *pdev )
{
	...
	netdev = (struct net_device *)alloc_etherdev(sizeof(EZT_NET_PRIV));
	if (!netdev)
		return -ENOMEM;
	...
	netdev->netdev_ops     = &nps_he0_netdev_ops;
	...
	REG_GEMAC_MAC_CFG = GEMAC_CFG_INIT_VALUE;
	REG_GEMAC_RX_CTL = 0;
	ez_net_hw_disable_control();
	...
	/* irq Rx allocation */
	err = request_irq( IRQ_LAN_RX_LINE, ez_net_rx_irq, 0 ,netdev->name,netdev );
	if ( err ) {
	...
	/* irq Tx allocation */
	err = request_irq( IRQ_LAN_TX_LINE, ez_net_tx_irq, 0 ,netdev->name,netdev );
	if ( err ) {
		netif_err(netPriv, probe, netdev,
				"EZnet: Fail to allocate IRQ %d - err %d\n",
				IRQ_LAN_TX_LINE, err);
	...
	if (err != 0) {
		netif_err(netPriv, probe, netdev,
				...
				__FUNCTION__, netdev->name, (int)err);
		err = -ENODEV;
		goto ez_net_probe_error;
	}
	return 0;
...
static int __devexit ez_net_remove( struct platform_device *pdev )
{
	...
	if ( !pdev ) {
		printk( KERN_ERR "%s: Can not remove undefined device\n",__FUNCTION__ );
		return -EINVAL;
	}
	...
	free_irq( IRQ_LAN_RX_LINE, npshe0dev );
	free_irq( IRQ_LAN_TX_LINE, npshe0dev );
...
static struct platform_driver npshe0_lan_driver = {
   .probe      = ez_net_probe,
...
static int __init npshe0_enet_init_module(void)
{
	int rc;

	printk_init(KERN_INFO "%s", version);
	rc = platform_driver_register(&npshe0_lan_driver);
	if ( rc ) {
...
static void __exit npshe0_enet_cleanup_module(void)
{
   platform_driver_unregister(&npshe0_lan_driver);
}


module_exit(npshe0_enet_cleanup_module);
module_init(npshe0_enet_init_module);
