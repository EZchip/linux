...
#define CONFIG_NR_NPS_HW_THREADS	1
#endif

#define NR_CORES	(NR_CPUS/CONFIG_NR_NPS_HW_THREADS)
#define NPS_CORE_NUM_PER_CLUSTER		16
...
#define NPS_CPU_TO_CLUSTER(cpu)	\
		(cpu / (CONFIG_NR_NPS_HW_THREADS * \
					NPS_CORE_NUM_PER_CLUSTER))
...
#define NPS_CPU_TO_CORE_NUM(cpu)	(cpu % NPS_CORE_NUM_PER_CLUSTER)
#define NPS_CPU_TO_THREAD_NUM(cpu)	(cpu % CONFIG_NR_NPS_HW_THREADS)
#define NPS_CPU_MTM_ID(th)			((1 << (th >> 2)) + (th % 4))
...
#define NPS_IO_MEM_REG_THREAD(cpu, cl, id, offset) \
...
#define NPS_CPU_MTM_REGS(cpu, offset) \
	NPS_IO_MEM_REG_CPU(cpu,\
			NPS_CPU_MTM_ID(NPS_CPU_TO_THREAD_NUM(cpu)),\
			offset)
